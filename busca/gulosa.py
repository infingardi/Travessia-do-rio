...
def solve_greedy_search():
    i = 0
    root = Node(name=0, parent=None, state=initial_state, weight_state=get_state_weight(initial_state), weight_path=0, is_solution = False)
    open_nodes = [root]
    solution = None
    while open_nodes:
        ...
        for next_state, rule in possible_states:
          ...
          insert_ordered(open_nodes, node, lambda n: n.weight_state)
